from apis.shorts_calculation.serializers import CalculationDataSerializer, \
    SyncMotorSerializer, AsyncMotorSerializer, TransformerSerializer, \
    AutoTransformerSerializer, GeneratorSerializer
from rest_framework import viewsets, status
from rest_framework.decorators import action
from apis.shorts_calculation.calculation_helpers import ShortsScheme
from rest_framework.response import Response
from apis.permissions import IsApparatusAdderOrReadOnly
from apparatus.models import Transformer, Generator, SyncMotor, AsyncMotor, \
    AutoTransformer
from rest_framework.permissions import SAFE_METHODS


class ShortsCalculationViewSet(viewsets.GenericViewSet):
    serializer_class = CalculationDataSerializer

    @action(detail=False, methods=('post',))
    def calculate_shorts(self, request):
        data = request.data
        serializer = self.get_serializer(data=data)
        if serializer.is_valid(raise_exception=True):
            network = serializer.validated_data['network']
            vertices = set()
            for elem in network:
                vertices.add(elem['startpoint'])
                vertices.add(elem['endpoint'])
            scheme = ShortsScheme(len(set(vertices)))
            equivalent_circuit = scheme.make_substitution_scheme(network)
            result_scheme, short_current = scheme.calculate_short_circuit(
                network,
                serializer.validated_data['calculation_point']
            )
            return Response(
                {'equivalent_circuit': equivalent_circuit,
                 'result_scheme': result_scheme,
                 'short_current': short_current},
                status=status.HTTP_200_OK
            )
        else:
            return Response(
                serializer.errors,
                status=status.HTTP_400_BAD_REQUEST
            )


class ApparatusViewSet(viewsets.ModelViewSet):
    permission_classes = (IsApparatusAdderOrReadOnly,)

    def get_queryset(self):
        instance_type = self.request.query_params.get('instance_type')
        type_to_model = {
            'transformer': Transformer,
            'autotransformer': AutoTransformer,
            'generator': Generator,
            'syncmotor': SyncMotor,
            'asyncmotor': AsyncMotor
        }
        model = type_to_model.get(instance_type)
        return model.objects.all()

    def get_serializer_class(self):
        instance_type = self.request.query_params.get('instance_type')
        type_to_serializer = {
            'transformer': TransformerSerializer,
            'autotransformer': AutoTransformerSerializer,
            'generator': GeneratorSerializer,
            'syncmotor': SyncMotorSerializer,
            'asyncmotor': AsyncMotorSerializer
        }
        return type_to_serializer.get(instance_type)

    def get_serializer(self, *args, **kwargs):
        if self.request.method not in SAFE_METHODS:
            kwargs['data'].update({
                'added_by': self.request.user.id,
                'is_moderated': self.request.user.is_staff
            })
        serializer_class = self.get_serializer_class()
        kwargs.setdefault('context', self.get_serializer_context())
        return serializer_class(*args, **kwargs)
